...
line_mode = False      # Activate link mode or desactivate it
line_start = None      # Save the first click
...
#Ligne mode Message
def messageclick():
    global line_mode, line_start
    line_mode = True
    line_start = False
    canvas["cursor"] = "cross"  # Cross cursor
    messagebox.showinfo("Line Mode", "Put 2 points to Create a Line")
...
def LinkPoints(event):
    ...
    if not line_start:
        ...
        # Second click for Link the 2 points
        x1, y1 = line_start
        x2, y2 = event.x, event.y

        canvas.create_line(x1, y1, x2, y2, fill=penColor, width=4)
...
class untitled(App):
    def main(self):
        vbox = gui.VBox(width=300, height=250)
...
# # Tool 9 - Decreament by 1
# link = Button(
#     holder, text="Link", height=1, width=12, command=askShapeDimention
# )
# link.grid(row=2, column=4)
#------------------------------------------------------------------------------------#

# Tool 9 - Decreament by 1
link = Button(
    holder, text="Link", height=1, width=12, command=messageclick
)
link.grid(row=2, column=4)
...
canvas.bind("<Button-1>", LinkPoints, add="+")
